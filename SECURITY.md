...
Handlers and scripts run with the worker's OS identity and access to its
resources. Process isolation contains trusted execution; it does not isolate
hostile code from the host. Run untrusted code under a separate OS identity or
...
application owns access controls, quotas and retention.
